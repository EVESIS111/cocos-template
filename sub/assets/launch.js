...
cc.Class({
    ...
    // 更新item - use cached component references instead of cc.find per item per update
    updateItem: function () {
        var THAT = this;
        /** 排行榜列表 */
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if (list[index]) {
                ...
                if (index < 3) {
                    ...
                    refs.cup.spriteFrame = THAT.cupImg[index];
                    refs.rank.node.active = false;
                } else {
                    refs.cup.node.active = false;
                    refs.rank.node.active = true;
                    refs.rank.string = index + 1;
                }
                // 头像
                refs.head.spriteFrame = list[index].spriteFrame;
        ...
        if (this.rank_list.length == 0) {
            ...
            this.none_data_label.string = '暂无数据...';
    ...
    init: function () {
        ...
        this.none_data_label = this.none_data.getComponent(cc.Label);
        /** 首个item */
        ...
        // Cache child component references once to avoid repeated cc.find in updateItem
        this.itemRefs = [];
        var children = this.item_box.children;
    ...
    /**
     * 对比版本 - cache system info to avoid calling wx.getSystemInfoSync every time
     * @param {string} v2 对比的版本
     */
    compareVersion: function (v2) {
        // Cache system SDK version on first call
        if (!this._cachedSDKVersion) {
            this._cachedSDKVersion = wx.getSystemInfoSync().SDKVersion;
        }
        var v1 = this._cachedSDKVersion.split('.');
        v2 = v2.split('.');
